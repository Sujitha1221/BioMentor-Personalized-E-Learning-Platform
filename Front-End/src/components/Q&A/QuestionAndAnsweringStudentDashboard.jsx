...
const QuestionAndAnsweringStudentDashboard = () => {
    ...
    if (!studentData) return <div className="text-center text-red-500 text-lg font-semibold">No Data Found</div>;
    ...
    return (
        <div className="p-6 bg-gray-100 min-h-screen w-full max-w-screen-xl mx-auto md:mt-20">
            ...
            >
                {[
                    { title: "Student ID", value: studentData.student_id, icon: <AiOutlineUser /> },
                    { title: "Total Evaluations", value: studentData.evaluations.length, icon: <AiOutlineBarChart /> },
                    { title: "Proficiency Level", value: "Intermediate", icon: <AiOutlineRise /> },
                    { title: "Last Evaluation", value: new Date(studentData.score_trends.timestamps.at(-1)).toLocaleDateString(), icon: <AiOutlineLineChart /> },
                ].map((item, index) => (
                    <motion.div
                        key={index}
                    ...
                    >
                        ...
                        <h3 className="text-gray-600 text-sm uppercase font-semibold mt-2">{item.title}</h3>
                        <motion.p
                            initial={{ opacity: 0, y: 10 }}
                            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
                <div className="bg-white p-6 rounded-xl shadow-md w-full hover:shadow-lg">
                    <h2 className="text-xl font-semibold text-gray-700 mb-4">📊 Performance Breakdown</h2>
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={[
                            { metric: "Semantic", score: studentData.average_scores.semantic_score },
                            { metric: "TF-IDF", score: studentData.average_scores.tfidf_score },
                            { metric: "Jaccard", score: studentData.average_scores.jaccard_score },
                            { metric: "Grammar", score: studentData.average_scores.grammar_score }
                        ]}>
                ...
                <div className="bg-white p-6 rounded-xl shadow-md w-full hover:shadow-lg">
                    <h2 className="text-xl font-semibold text-gray-700 mb-4">📈 Score Trends Over Time</h2>
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={studentData.score_trends.timestamps.map((date, index) => ({
                            date,
                            ...
                            <Legend />
                            <Line type="monotone" dataKey="semantic" stroke="#2563EB" strokeWidth={3} />
                            <Line type="monotone" dataKey="tfidf" stroke="#10B981" strokeWidth={3} />
                            <Line type="monotone" dataKey="jaccard" stroke="#F59E0B" strokeWidth={3} />
                            <Line type="monotone" dataKey="grammar" stroke="#EF4444" strokeWidth={3} />
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            </div>
                        ...
                        <div className="mt-4 space-y-2">
                            {studentData.feedback_report.weaknesses.map((weakness, index) => (
                                <div key={index} className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        <div className="text-gray-700 font-semibold">{weakness}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-md w-full mt-8">
                <h2 className="text-xl font-semibold text-gray-700 mb-4">📊 Peer Comparison</h2>
                <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={[
                        { metric: "Semantic", student: studentData.average_scores.semantic_score, class: studentData.class_average?.semantic_score || 0 },
                        { metric: "TF-IDF", student: studentData.average_scores.tfidf_score, class: studentData.class_average?.tfidf_score || 0 },
                        { metric: "Jaccard", student: studentData.average_scores.jaccard_score, class: studentData.class_average?.jaccard_score || 0 },
                        { metric: "Grammar", student: studentData.average_scores.grammar_score, class: studentData.class_average?.grammar_score || 0 }
                    ]}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="metric" />
                        <YAxis />
